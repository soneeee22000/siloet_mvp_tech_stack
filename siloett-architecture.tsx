...
import { Database, Cpu, Globe, FileText, CheckCircle, AlertCircle, ArrowDown, Search, Zap } from 'lucide-react';

const SiloettArchitecture = () => {
  ...
  const layers = [
    {
      ...
      icon: Globe,
      tech: ['React (v0.dev)', 'Vercel Deployment'],
  ...
  const dataFlowSteps = [
    { from: 'User', to: 'UI', label: 'Submit script or query', color: 'blue' },
    { from: 'UI', to: 'API', label: 'REST API call', color: 'purple' },
    { from: 'API', to: 'RAG', label: 'Trigger RAG pipeline', color: 'green' },
    { from: 'RAG', to: 'DB', label: 'Search embeddings', color: 'orange' },
    { from: 'DB', to: 'RAG', label: 'Return canon docs', color: 'orange' },
    { from: 'RAG', to: 'API', label: 'Generated response + citations', color: 'green' },
    { from: 'API', to: 'UI', label: 'Format & display', color: 'purple' },
    { from: 'UI', to: 'User', label: 'Show results', color: 'blue' }
  ];
          ...
          <div className="bg-gray-800 rounded-xl p-8 border border-gray-700">
            ...
            <div className="space-y-3">
              {dataFlowSteps.map((step, index) => (
                <div key={index} className="flex items-center gap-4 animate-fadeIn" style={{ animationDelay: `${index * 100}ms` }}>
